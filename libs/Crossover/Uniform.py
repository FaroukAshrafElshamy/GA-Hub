...
def Uniform():
    ...
    window1 = ttk.Window()
    window1.title("Uniform Crossover")
    ...
    Photo = PhotoImage(master = window1, file = r"libs\assets\Uniform1.png")
    label = ttk.Label(master= window1, image= Photo)
    label.place(x=140 , y=200)

    Photo1 = PhotoImage(master = window1, file = r"libs\assets\Uniform2.png")
    ...
    canvas.create_text(
        ...
        anchor="nw",
        text="""
    Uniform Crossover: Each gene is selected randomly from one of the corresponding genes 
    of the parent chromosomes, Use random generated mask.
            """,
        fill="#0063BC",
        font=("K2D Medium", 20 * -1)
    )
    ...
    canvas.create_text(
        ...
        40,
        anchor="nw",
        text="How does Uniform crossover work?",
        fill="#000000",
